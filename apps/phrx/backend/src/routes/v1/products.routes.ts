...
import { servicoIdParamSchema } from "../../modules/tenant/products/application/dto/servico.dto";
...
function registerServicoResource(router: Router, path: string): void {
  router.get(
    path,
    tenantAuthMiddleware(),
    tenantBranchContextMiddleware(),
    requirePermission("PRODUTOS", "VIEW"),
    async (context) => servicoController.search(context.req),
  );
  ...
  router.post(
    ...
    auditMiddleware,
    async (context) =>
      servicoController.create(context.req, getTenantAuth(context).userId),
  );
}

function registerServicoItemResource(router: Router, path: string): void {
  router.get(
    path,
    tenantAuthMiddleware(),
    tenantBranchContextMiddleware(),
    ...
    async (context) => {
      ...
      return servicoController.get(servicoId);
    },
  );
  ...
  router.put(
    ...
    async (context) => {
      const { servicoId } = parseRouteParams(context.params, servicoIdParamSchema);
      return servicoController.update(
        servicoId,
        context.req,
        getTenantAuth(context).userId,
      );
...
export function registerProductRoutes(router: Router, prefix: string): void {
  ...
  registerServicoResource(router, `${prefix}/tenant/servicos`);
  registerServicoResource(router, `${prefix}/tenant/services`);
  router.get(
    `${prefix}/tenant/servicos/stats`,
    tenantAuthMiddleware(),
    tenantBranchContextMiddleware(),
    requirePermission("PRODUTOS", "VIEW"),
    async () => servicoController.stats(),
  );
  registerServicoItemResource(router, `${prefix}/tenant/servicos/:servicoId`);
  registerServicoItemResource(router, `${prefix}/tenant/services/:servicoId`);
